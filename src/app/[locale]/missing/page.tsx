import type { Metadata } from "next";
import Image from "next/image";
import Link from "next/link";
import { notFound } from "next/navigation";
import { Button } from "@/components/ui/button";
import type { Locale } from "@/config/site";
import { isValidLocale, localePath } from "@/config/i18n";
import { getDictionary } from "@/lib/dictionary";

type PageProps = { params: Promise<{ locale: string }> };

export async function generateMetadata({
  params,
}: PageProps): Promise<Metadata> {
  const { locale: raw } = await params;
  if (!isValidLocale(raw)) return {};
  const dictionary = getDictionary(raw as Locale);
  return {
    title: dictionary.common.notFoundTitle,
    description: dictionary.common.notFoundBody,
    robots: {
      index: false,
      follow: true,
      googleBot: { index: false, follow: true },
    },
  };
}

/** Routable 404 shell for middleware rewrites that must return HTTP 404. */
export default async function MissingPage({ params }: PageProps) {
  const { locale: raw } = await params;
  if (!isValidLocale(raw)) notFound();
  const locale = raw as Locale;
  const dictionary = getDictionary(locale);

  return (
    <div className="flex min-h-[70vh] flex-col items-center justify-center bg-atmosphere px-6 py-16 text-center">
      <div className="relative mb-8 aspect-[16/10] w-full max-w-lg">
        <Image
          src="/images/404.webp"
          alt=""
          fill
          className="object-contain"
          sizes="512px"
          priority
        />
      </div>
      <h1 className="text-4xl font-bold tracking-tight text-foreground">
        {dictionary.common.notFoundTitle}
      </h1>
      <p className="mt-4 max-w-md text-muted-foreground">
        {dictionary.common.notFoundBody}
      </p>
      <div className="df-actions mt-8">
        <Button asChild size="lg">
          <Link href={localePath(locale)}>{dictionary.common.notFoundHome}</Link>
        </Button>
        <Button asChild size="lg" variant="secondary">
          <Link href={localePath(locale, "/blog")}>
            {dictionary.common.notFoundBlog}
          </Link>
        </Button>
      </div>
    </div>
  );
}
